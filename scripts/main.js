...
renderStack.push(function () {
  if (isZooming) {
    camera.position.lerp(zoomTarget, 0.08); // Speed of zoom
...
function onDocumentMouseDown(event) {
  ...
  if (intersects.length > 0) {
    zoomTarget.copy(intersects[0].point).normalize().multiplyScalar(0.52);
    isZooming = true;
    controls.enabled = false; // Disable interaction during zoom
  }
}
